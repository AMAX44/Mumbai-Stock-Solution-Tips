...
import 'package:mumbai_stock_solution/datamodels/contact_model.dart';
...
class AdminContactDetails extends StatefulWidget {
  const AdminContactDetails({super.key});

  @override
  State<AdminContactDetails> createState() => _AdminContactDetailsState();
}

class _AdminContactDetailsState extends State<AdminContactDetails> {
  late Query _contactQuery =
  ...
  void initState() {
    setState(() {
      _contactQuery = FirebaseFirestore.instance.collection('contactDetails');
    });
    super.initState();
  }
            ...
            onTap: () {
              Navigator.push(
                ...
                MaterialPageRoute<void>(
                  builder: (BuildContext context) => AdminRiskProfile(),
                ),
              );
            },
            child: const Text(
  ...
  Future<void> downloadReport() async {
    ...
    QuerySnapshot? snapshot = await _contactQuery.get();
      ...
      for (var headerIndex = 0;
        ...
        switch (headerData.elementAt(headerIndex)) {
          ...
          case 'Segment':
            _addTextCell(
              range: range,
              text: data.segment != null ? '${data.segment}' : '',
            );
            break;
          case 'Investment':
            _addTextCell(
              range: range,
              text: data.investment != null ? '${data.investment}' : '',
            );
            break;
        }
      }
    ...
    workbook.dispose();
    await saveAndLaunchFile(bytes, 'Contact Details Data.xlsx');
    isLoading = false;
  }
